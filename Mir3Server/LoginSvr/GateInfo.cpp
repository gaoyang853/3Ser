...
void CGateInfo::ProcSelectServer(SOCKET s, WORD wServerIndex)
{
	...
	char				szEncodePacketPort[12];//fire
	...
	while (pListNode)
	{
		...
		if (pUserInfo->sock == s)
		{
			if (!pUserInfo->fSelServerOk)
			{
				...
				ZeroMemory(cTemp, 6);
				cTemp[0] = '/';
				itoa(7100, &cTemp[1], 10);
				cTemp[5] = '\0';
				int nPos3 = fnEncode6BitBufA((unsigned char *)cTemp, szEncodePacketPort, 6, sizeof(szEncodePacketPort));

				memmove(szEncodeAllPacket, szEncodeMsg, nPos);
				memmove(&szEncodeAllPacket[nPos], szEncodePacket, nPos2);
				memmove(&szEncodeAllPacket[nPos + nPos2], szEncodePacketPort, nPos3);//fire

				szEncodeAllPacket[nPos + nPos2 + nPos3] = '\0';
